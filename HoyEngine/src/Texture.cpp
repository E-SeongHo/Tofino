...
namespace Tofino
{
    ...
    {
        m_filename = filename;
        std::string format = filename.substr(filename.rfind('.') + 1);

        if (format == "dds")
        {
            std::wstring wfilename = std::wstring().assign(filename.begin(), filename.end());
            ...
            ThrowIfFailed(DirectX::CreateDDSTextureFromFileEx(
                ...
                DirectX::DDS_LOADER_FLAGS(false), (ID3D11Resource**)m_texture.GetAddressOf(),
