...
Graphics::Graphics(HWND hWnd, const int screenWidth, const int screenHeight)
    :m_window(hWnd), m_width(screenWidth), m_height(screenHeight), m_screenViewport(D3D11_VIEWPORT()), triangleIndexCount(0)
{
}
...
void Graphics::CreateConstantBuffer(const ConstantBuffer& cbufferData, ComPtr<ID3D11Buffer>& constantBuffer)
{
...
bool Graphics::Init()
{
    Graphics::InitD3D(m_width, m_height);
...
void Graphics::Render()
{
    ...
    m_context->VSSetConstantBuffers(0, 1, triangleConstantBuffer.GetAddressOf());
    ...
    m_context->IASetVertexBuffers(0, 1, triangleBuffer.GetAddressOf(), &stride, &offset);
    m_context->IASetIndexBuffer(triangleIndexBuffer.Get(), DXGI_FORMAT_R16_UINT, 0);
    m_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    m_context->DrawIndexed(triangleIndexCount, 0, 0);
}
